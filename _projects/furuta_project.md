...
- The rotary arm (actuated, angle \( \theta \)).  
- The pendulum rod (unactuated, angle \( \alpha \)).
...
The **\( Q \)** and **\( R \)** matrices in LQR are chosen to balance state error (e.g., penalizing large deviations of \( \alpha \) from \( \pi \)) and control effort. The resulting LQR gain matrix \( \mathbf{K} \) yields 
...
2. **Stabilization**  
   ...
   - Minimizes a cost function 
     \[
       J = \int_0^{\infty} \bigl(\mathbf{x}^T Q \,\mathbf{x} \;+\; u^T R\,u\bigr)\, dt.
     \]
